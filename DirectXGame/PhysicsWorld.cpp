...
float PhysicsWorld::closestDistanceBetweenLines(Vector3& p0,  Vector3& p1, const Vector3& u, const Vector3& v, float s0, float s1)
{
    ...
    float t = -VectorDot(VectorCross(p1-p0, u), uv)/uvSquared;
    float s = -VectorDot(VectorCross(p1-p0, v), uv)/uvSquared;
    if(abs(t)>s1 || abs(s)>s0 || EpsilonEqual(t, 0.0f, 0.01f) || EpsilonEqual(s, 0.0f, 0.01f))
        return std::numeric_limits<float>::max();

    return abs(VectorDot(uv, p0-p1)/VectorLength(uv));
}
...
bool PhysicsWorld::closestPointsDoIntersect(Vector3& p0,  Vector3& p1,  const Vector3& u, const Vector3& v, float s0, float s1, float max1, float max2)
{
    ...
    float t = -VectorDot(VectorCross(p1-p0, u), uv)/uvSquared;
    float s = -VectorDot(VectorCross(p1-p0, v), uv)/uvSquared;
    float dist = abs(VectorDot(uv, p0-p1)/VectorLength(uv));
    if(abs(t)>s1 || abs(s)>s0  || std::isnan(t) || std::isnan(s))
        return false;
...
bool PhysicsWorld::detectCubeSphereCollision(float dt, BoxCollider* cube, SphereCollider* sphere, ContactInfo& contactInfo)
{
    ...
    float penetration = abs(VectorDot(aX, T)) - cube->xSize - sphere->radius;
    if(penetration > 0)
    {
    ...
    penetrationX = abs(penetration);

    penetration = abs(VectorDot(aY, T)) - cube->ySize - sphere->radius;
    if(penetration > 0)
    {
        return false;
    }

    penetrationY = abs(penetration);
    ...
    penetration = abs(VectorDot(aZ, T)) - cube->zSize - sphere->radius;
    if(penetration > 0)
    {
        return false;
    }
    ...
    if(abs(VectorDot(aX, T))<cube->xSize && abs(VectorDot(aZ, T))<cube->zSize && abs(VectorDot(aY, T))<cube->ySize)
    {
...
bool PhysicsWorld::detectCubeCubeCollision(float dt, BoxCollider* cubeA, BoxCollider* cubeB, ContactInfo& contactInfo)
{
    ...
    float penetration = abs(VectorDot(T, aX)) - (cubeA->xSize + abs(cubeB->xSize*rxx) + abs(cubeB->ySize*rxy) + abs(cubeB->zSize*rxz));
    ...
    penetration = abs(VectorDot(T, aY)) - (cubeA->ySize + abs(cubeB->xSize*ryx) + abs(cubeB->ySize*ryy) + abs(cubeB->zSize*ryz));
    ...
    penetration = abs(VectorDot(T, aZ)) - (cubeA->zSize + abs(cubeB->xSize*rzx) + abs(cubeB->ySize*rzy) + abs(cubeB->zSize*rzz));
    ...
    penetration = abs(VectorDot(T, bX)) - (abs(cubeA->xSize*rxx) + abs(cubeA->ySize*ryx) + abs(cubeA->zSize*rzx) + cubeB->xSize);
    ...
    penetration = abs(VectorDot(T, bY)) - (abs(cubeA->xSize*rxy) + abs(cubeA->ySize*ryy) + abs(cubeA->zSize*rzy) + cubeB->ySize);
    ...
    penetration = abs(VectorDot(T, bZ)) - (abs(cubeA->xSize*rxz) + abs(cubeA->ySize*ryz) + abs(cubeA->zSize*rzz) + cubeB->zSize);
    ...
    penetration = abs(VectorDot(T, aZ)*ryx - VectorDot(T, aY)*rzx) - (abs(cubeA->ySize*rzx)+abs(cubeA->zSize*ryx)+abs(cubeB->ySize*rxz)+abs(cubeB->zSize*rxy));
    if(penetration>0)
    {
        return false;
    }
    ...
    penetration = abs(VectorDot(T, aZ)*ryy - VectorDot(T, aY)*rzy) - (abs(cubeA->ySize*rzy)+abs(cubeA->zSize*ryy)+abs(cubeB->xSize*rxz)+abs(cubeB->zSize*rxx));
    if(penetration>0)
    {
        return false;
    }
    ...
    penetration = abs(VectorDot(T, aZ)*ryz - VectorDot(T, aY)*rzz) - (abs(cubeA->ySize*rzz)+abs(cubeA->zSize*ryz)+abs(cubeB->xSize*rxy)+abs(cubeB->ySize*rxx));
    if(penetration>0)
    {
        return false;
    }
    ...
    penetration = abs(VectorDot(T, aX)*rzx - VectorDot(T, aZ)*rxx) - (abs(cubeA->xSize*rzx)+abs(cubeA->zSize*rxx)+abs(cubeB->ySize*ryz)+abs(cubeB->zSize*ryy));
    if(penetration>0)
    {
        return false;
    }
    ...
    penetration = abs(VectorDot(T, aX)*rzy - VectorDot(T, aZ)*rxy) - (abs(cubeA->xSize*rzy)+abs(cubeA->zSize*rxy)+abs(cubeB->xSize*ryz)+abs(cubeB->zSize*ryx));
    if(penetration>0)
    {
        return false;
    }
    ...
    penetration = abs(VectorDot(T, aX)*rzz - VectorDot(T, aZ)*rxz) - (abs(cubeA->xSize*rzz)+abs(cubeA->zSize*rxz)+abs(cubeB->xSize*ryy)+abs(cubeB->ySize*ryx));
    if(penetration>0)
    {
        return false;
    }
    ...
    penetration = abs(VectorDot(T, aY)*rxx - VectorDot(T, aX)*ryx) - (abs(cubeA->xSize*ryx)+abs(cubeA->ySize*rxx)+abs(cubeB->ySize*rzz)+abs(cubeB->zSize*rzy));
    if(penetration>0)
    {
        return false;
    }
    ...
    penetration = abs(VectorDot(T, aY)*rxy - VectorDot(T, aX)*ryy) - (abs(cubeA->xSize*ryy)+abs(cubeA->ySize*rxy)+abs(cubeB->xSize*rzz)+abs(cubeB->zSize*rzx));
    if(penetration>0)
    {
        return false;
    }
    ...
    penetration = abs(VectorDot(T, aY)*rxz - VectorDot(T, aX)*ryz) - (abs(cubeA->xSize*ryz)+abs(cubeA->ySize*rxz)+abs(cubeB->xSize*rzy)+abs(cubeB->ySize*rzx));
    if(penetration>0)
    {
        return false;
    }
...
bool PhysicsWorld::isCubeCubePetrusion(const Vector3& normal, const std::vector<Vector3>& points, BoxCollider* toCube, BoxCollider::ContactDir dir)
{
    ...
    for(Vector3 point: points)
    {
        ...
        if(d>=-tolerance)
        {
            ...
            float dist1 = abs(VectorDot(intersectionPoint-p0, adj1));
            float dist2 = abs(VectorDot(intersectionPoint-p0, adj2));
...
bool PhysicsWorld::sphereRaycast(const Vector3& start, const Vector3& dir, RayCastData& dat, SphereCollider* sphere)
{
    ...
    if(AllEqual(dir, Vector3(0,1,0))  || AllEqual(dir, Vector3(0,-1,0)))
    {
        up = Vector3(0,0,1);
    }
    else
    ...
    if(dist>=0.0f)
    {
        ...
        if(l2<=sphere->radius*sphere->radius)
        {
            ...
            float theta = asin(length/sphere->radius);
            Vector3 dirComp = normal*length/tan(theta);
            if(length==0.0f)
...
bool PhysicsWorld::cubeRaycast(const Vector3& start, const Vector3& dir, RayCastData& dat, BoxCollider* cube)
{
    ...
    for(int i = 0;i<6;i++)
    {
        ...
        {
            Vector3 intersection = start + dir*dist;
            float dist1 = abs(VectorDot(intersection-p0, adj1));
            float dist2 = abs(VectorDot(intersection-p0, adj2));
...
void PhysicsWorld::cubeCubeCollisionResponseDynamicVsStatic(ContactInfo& info, const Vector3& norm, float dt, BoxCollider* dynamicCube, BoxCollider* staticCube)
{
    ...
    if(!dynamicCube->rb->restingContact)
    {
        ...
        for(int i =0;i<info.points.size();i++)
        {
            ...
            // qDebug() << "surface slope: " << abs(VectorDot(VectorNormalize(gravity), info.normal));
            if(abs(j)<0.5f && abs(VectorDot(VectorNormalize(gravity), info.normal))>0.7f)
            {
                dynamicCube->rb->restingContact = true;
    ...
    else
    {
        ...
        if(speed<0.1f && angularSpeed<0.1f)
        {
            ...
            if(abs(VectorDot(faceDir, info.normal))>0.975f)
            {
        ...
        else if(info.points.size())
        {
            ...
            for(int i =0;i<info.points.size();i++)
            {
                ...
                Vector3 tangentialForce = (VectorLength(vt)/VectorLength(vn))*abs(j)*VectorNormalize(vt)/(dt*info.points.size());
                // Utilities::PrintVec3(tangentialForce);
                float angularRel = VectorLength(dynamicCube->rb->angularVelocity-staticCube->rb->angularVelocity);

                //if(info.faceCollision)
                dynamicCube->rb->addTorque(VectorCross(dynamicCube->rb->mass*gravity, ra));
                if(j>0)
...
{
    Vector3 normal = VectorNormalize(info.normal);
    ...
    Vector3 force = -0.1f*normal*VectorDot(sphere->rb->velocity, normal)*sphere->rb->mass/dt;
    float j = VectorLength(force);
    float sMax = 5.0f*restitutionSlope*abs(VectorDot(gravity, normal))+restitutionIntersect;
